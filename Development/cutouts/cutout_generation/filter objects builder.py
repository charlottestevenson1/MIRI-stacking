...
# Bands in goods-s (35)
with open('development/filter_lists/filter list s') as f:
    bands_s = [band.strip() for band in f.readlines()]

# Bands in goods-n (27)
with open('development/filter_lists/filter list n') as f:
    bands_n = [band.strip() for band in f.readlines()]

# Bands in MIRI HDU (8)
with open('development/filter_lists/filter list miri') as f:
...
for band in bands_s:
    ...
    for j in ['goods-s', 'goods-n']:
        ...
        hdul = fits.open('fits_files/'+j+' catalog.fits')
    ...
    with open('development/filter_objects/'+band+' objects.txt', 'w') as f:
