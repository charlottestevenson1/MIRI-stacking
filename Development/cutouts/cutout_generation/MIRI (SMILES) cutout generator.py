...
BANDS = [i.strip() for i in open('development/filter_lists/filter list miri', 'r').readlines()]
#print(BANDS[26:])
# BAND = "F277W"
size = 5 * u.arcsec
...
for BAND in BANDS:
    ...
    with open("development/filter_objects/ALL MIRI.txt") as f:
    ...
    filename = f'fits_files/SMILES MIRI/hlsp_smiles_jwst_miri_goodss_{BAND.lower()}_v1.0_drz.fits'
    
    hdul = fits.open(filename)
    data = hdul[datatype].data

    coorddata = fits.open('fits_files/jades_small.fits')[2].data
    ...
    for ID in IDs:
        ...
        fits.writeto(
            f"development/cutouts/WHT/{ID}_{BAND}_WHT.fits",
            cutout.data,
            header = new_header,
            overwrite = True
        )
