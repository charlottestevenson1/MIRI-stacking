...
with open('Development/Photometry/Band data/Eff widths.txt') as f:
...
with open('Development/Photometry/Band data/Aperture corrections.txt') as f:
...
for z_range in z_ranges:
    zlo = z_range[0]
    zup = z_range[1]
    with open(f'Development/Redshift Bins/Stacks/Redshifts {zlo}-{zup}/Background levels.txt') as f:
        all_bkgs = np.array([float(bkg) for bkg in f.readlines()]) * MJysr_to_nJy

    with open(f'Development/Redshift Bins/Stacks/Redshifts {zlo}-{zup}/Background levels MAD.txt') as f:
    ...
    for i in range(len(bands)):
        ...
        image = fits.getdata(f'Development/Redshift Bins/Stacks/Redshifts {zlo}-{zup}/SCI/{band}_stack.fits')
        #error = fits.getdata(f'Development/Redshift Bins/Stacks/Redshifts {zlo}-{zup}/ERR/{band}_stack_ERR.fits')
    ...
    with open(f'Development/Prospector/Stack data/{zlo}-{zup} Fluxes.txt', 'w') as f:
        for point in fluxes_BSUB_CORR:
            f.writelines(str(point)+'\n')

    with open(f'Development/Prospector/Stack data/{zlo}-{zup} Errors.txt', 'w') as f:
