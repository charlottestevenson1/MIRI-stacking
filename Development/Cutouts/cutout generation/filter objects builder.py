...
# Bands in goods-s (35)
with open('Development/Filter lists/filter list s') as f:
    bands_s = [band.strip() for band in f.readlines()]

# Bands in goods-n (27)
with open('Development/Filter lists/filter list n') as f:
    bands_n = [band.strip() for band in f.readlines()]

# Bands in MIRI HDU (8)
with open('Development/Filter lists/filter list miri') as f:
...
for band in bands_s:
    ...
    with open('Development/Filter objects/'+band+' objects.txt', 'w') as f:
