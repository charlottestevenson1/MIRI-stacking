...
BANDS = [BAND.strip() for BAND in open('Development/Filter lists/filter list s').readlines()[:26]]+['detection']
...
def plot_SED(ID):
    ...
    pivotlist = [float(line.strip())/1e4 for line in open('Development/Visual inspection figures/SED plotting code/pivot wave calculation/pivot_waves.txt', 'r').readlines()]
...
for ID in IDs:
    ...
    for i in range(27):
        ...
        if str(ID) in [ID.strip() for ID in open(f'Development/Filter objects/{BAND} objects.txt')]:
    ...
    plot_SED(ID)
    fig.canvas.draw()
    fig.savefig(f'Development/Visual inspection figures/2" images/{ID}_figure.pdf', bbox_inches='tight', pad_inches=0.02)
