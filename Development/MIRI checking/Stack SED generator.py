...
with open(f'Development/MIRI checking/{ID} Background levels.txt') as f:
    all_bkgs = np.array([float(bkg) for bkg in f.readlines()]) * MJysr_to_nJy

with open(f'Development/MIRI checking/{ID} Background levels MAD.txt') as f:
...
for i in range(len(bands)):
    ...
    try:
        image = fits.getdata(f'Development/Cutouts/SCI/{ID}_{band}.fits')
...
with open(f'Development/MIRI checking/{ID} fluxes.txt', 'w') as f:
    for band in bands:
        f.write(f'{band}: {fluxes_BSUB_CORR[bands.index(band)]}\n')

with open(f'Development/MIRI checking/{ID} errors.txt', 'w') as f:
    for band in bands:
...
plt.title(f'Development/MIRI checking/{ID} SED')
plt.xscale('log')
plt.yscale('log')
