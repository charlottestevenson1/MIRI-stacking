...
for band in bands:
    filepath = f'Development/Photometry/Band data/profile data/{band}.dat'
...
with open('Development/Photometry/Band data/pivot_waves.txt', 'w') as f:
    for line in pwaves:
        f.writelines(str(line/1e4)+'\n') # conversion to microns

with open('Development/Photometry/Band data/Eff widths.txt', 'w') as f:
    for line in widths:
        f.writelines(str(line/1e4)+'\n')