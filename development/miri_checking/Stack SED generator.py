...
with open(f'development/miri_checking/{ID} Background levels.txt') as f:
    all_bkgs = np.array([float(bkg) for bkg in f.readlines()]) * MJysr_to_nJy

with open(f'development/miri_checking/{ID} Background levels MAD.txt') as f:
...
with open(f'development/miri_checking/{ID} fluxes.txt', 'w') as f:
    for band in bands:
        f.write(f'{band}: {fluxes_BSUB_CORR[bands.index(band)]}\n')

with open(f'development/miri_checking/{ID} errors.txt', 'w') as f:
    for band in bands:
...
plt.title(f'development/miri_checking/{ID} SED')
plt.xscale('log')
plt.yscale('log')
