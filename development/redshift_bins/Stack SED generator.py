...
with open('development/photometry/band_data/Eff widths.txt') as f:
...
with open('development/photometry/band_data/Aperture corrections.txt') as f:
...
for z_range in z_ranges:
    zlo = z_range[0]
    zup = z_range[1]
    with open(f'development/redshift_bins/stacks/Redshifts {zlo}-{zup}/Background levels.txt') as f:
        all_bkgs = np.array([float(bkg) for bkg in f.readlines()]) * MJysr_to_nJy

    with open(f'development/redshift_bins/stacks/Redshifts {zlo}-{zup}/Background levels MAD.txt') as f:
    ...
    for i in range(len(bands)):
        ...
        image = fits.getdata(f'development/redshift_bins/stacks/Redshifts {zlo}-{zup}/SCI/{band}_stack.fits')
        #error = fits.getdata(f'development/redshift_bins/stacks/Redshifts {zlo}-{zup}/ERR/{band}_stack_ERR.fits')
    ...
    with open(f'development/prospector/stack_data/{zlo}-{zup} Fluxes.txt', 'w') as f:
        for point in fluxes_BSUB_CORR:
            f.writelines(str(point)+'\n')

    with open(f'development/prospector/stack_data/{zlo}-{zup} Errors.txt', 'w') as f:
    ...
    fig.savefig(
        f"development/presentations/z={zlo}-{zup} Stack SED.png",
        dpi=400,
        bbox_inches="tight",
        transparent=True
    )
