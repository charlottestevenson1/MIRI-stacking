...
BANDS = [BAND.strip() for BAND in open('development/filter_lists/filter list s').readlines()[:26]]+['detection']

IDs = [int(id) for id in open('development/stacking/Objects for stacking.txt').readlines()]
...
def plot_SED(ID):
    ...
    pivotlist = [float(line.strip())/1e4 for line in open('development/visual_inspection_figures/sed_plotting_code/pivot_wave_calculation/pivot_waves.txt', 'r').readlines()]
...
for ID in IDs:
    ...
    for i in range(27):
        ...
        if str(ID) in [ID.strip() for ID in open(f'development/filter_objects/{BAND} objects.txt')]:
            hdul = fits.open(f'development/cutouts/SCI/{ID}_{BAND}.fits')
            bigdata = hdul[0].data
            nx, ny = bigdata.shape
    ...
    plot_SED(ID)
    fig.canvas.draw()
    fig.savefig(f'development/visual_inspection_figures/2" images/{ID}_figure.pdf', bbox_inches='tight', pad_inches=0.02)
