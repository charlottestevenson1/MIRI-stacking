...
run_params["outfile"] = "Final/6. Prospector/Fits/unspecified outfile.h5"
...
def build_obs(zlo=zlo, zup=zup):
    ...
    with open(f'Final/6. Prospector/Stack data/{zlo}-{zup} Fluxes.txt') as f:
        fluxes_nJy = [float(point) for point in f.readlines()]

    with open(f'Final/6. Prospector/Stack data/{zlo}-{zup} Errors.txt') as f:
...
def plot_results(run_params, model, sps):
    ...
    os.makedirs(f'Final/6. Prospector/Plots/z={zlo}-{zup}, {MIRI_desc}', exist_ok=True)
    ...
    plt.savefig(f'Final/6. Prospector/Plots/z={zlo}-{zup}, {MIRI_desc}/z={zlo}-{zup} {MIRI_desc} traceplot.png', dpi=300)
    #plt.show()
    plt.close()
    ...
    plt.savefig(f'Final/6. Prospector/Plots/z={zlo}-{zup}, {MIRI_desc}/z={zlo}-{zup} {MIRI_desc} cornerplot.png', dpi=300)
    #plt.show()
    plt.close()
    ...
    if results_type == "dynesty":
        ...
        plt.savefig(f'Final/6. Prospector/Plots/z={zlo}-{zup}, {MIRI_desc}/z={zlo}-{zup} {MIRI_desc} SFH.png', dpi=300)
        #plt.show()
        plt.close()
    ...
    plt.savefig(
        f"Final/6. Prospector/Plots/z={zlo}-{zup}, {MIRI_desc}/"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--readfile", type=str, default=f'Final/6. Prospector/Fits/z={zlo}-{zup} {MIRI_desc}.h5')
