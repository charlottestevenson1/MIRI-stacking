...
# Send email when fitting has completed? NOTE: You must set up your .env file with your email, password, and server for this to work. See the README for more information.
...
run_params["outfile"] = "final/06_sed_fitting/fits/unspecified_outfile.h5"
...
# 2) build_obs
# -----------------------
def build_obs(z_lo=z_lo, z_up=z_up):
    ...
    obs["phot_mask"] = np.array([True for i in range(len(filternames))]) if miri_inc else np.array([True for i in range(len(nircam_bands))] + [False for i in range(len(miri_bands))]) 
    #print(obs["phot_mask"])
...
# -----------------------
# 5) z_logsfr_ratios_to_masses
# -----------------------
def z_logsfr_ratios_to_masses(logmass=None, logsfr_ratios=None, zred=None, **extras):
    '''
...
def build_model(nbins_sfh=8, **kwargs):
    ...
    model_params['zred']['init'] = 0.5*(z_lo+z_up)
    model_params['zred']['prior'] = priors.TopHat(mini=z_lo, maxi=z_up)
    print(f'Redshift range: z={z_lo}-{z_up}\n{miri_desc}\n')
    ...
    model_params['mass']['depends_on'] = z_logsfr_ratios_to_masses
...
def plot_results(run_params, model, sps):
    ...
    os.makedirs('final/06_sed_fitting/plots', exist_ok=True)
    ...
    if results_type == "dynesty":
        ...
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.set_xlim([1, 10**(np.log10(cosmo.age(z_lo).value*1e3))])
        ax.invert_xaxis()
    ...
    plt.savefig(
        f"final/06_sed_fitting/plots/"
        f"z_{z_lo}_{z_up}_{miri_desc.replace(' ', '_').lower()}_sed.png",
        dpi=300,
        bbox_inches="tight"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--readfile", type=str, default=f'final/06_sed_fitting/fits/z={z_lo}-{z_up}_{miri_desc}.h5')
    ...
    obs = build_obs(z_lo=z_lo, z_up=z_up)
    ...
    if run_params["fitnewmodel"]:
        ...
        outfile_root, _ = os.path.splitext(run_params["outfile"])
        with open(f'{outfile_root}_generation_time.txt', 'w') as f:
