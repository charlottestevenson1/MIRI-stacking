...
title = f"z = {zlo}-{zup} ({MIRI_desc}) with {cut_param} {gt_or_lt} {cut_value}"

# Number of posterior samples used to generate SEDs
#
# Increase this for smoother percentile bands.
n_sed_samples = 2000
...
# Colours
# USE RED FOR MINORITY, BLUE FOR MAJORITY
...
redshift_bins = {
    "z = 8-9": f"Prospector/Fits/z=8-9 {MIRI_desc}.h5",
    "z = 9-10": f"Prospector/Fits/z=9-10 {MIRI_desc}.h5",
    "z = 10-11": f"Prospector/Fits/z=10-11 {MIRI_desc}.h5",
    "z = 11-12": f"Prospector/Fits/z=11-12 {MIRI_desc}.h5",
    "z = 12-15": f"Prospector/Fits/z=12-15 {MIRI_desc}.h5",
}
...
readfile = f'Prospector/Fits/z={zlo}-{zup} {MIRI_desc}.h5'
print(f'Readfile: {readfile}')

# ============================================================
# LOAD RESULTS AND SET MASK
# ============================================================
...
sample1_name = f"<= {cut_value} sample"
sample2_name = f"> {cut_value} sample"

mask1 = result["chain"][:, result["theta_labels"].index(cut_param)] <= cut_value
mask2 = result["chain"][:, result["theta_labels"].index(cut_param)] > cut_value
...
full_sed_mag_p16 = maggies_to_ab(full_sed_p84)
full_sed_mag_p50 = maggies_to_ab(full_sed_p50)
full_sed_mag_p84 = maggies_to_ab(full_sed_p16)

selected_sed_mag_p16 = maggies_to_ab(selected_sed_p84)
selected_sed_mag_p50 = maggies_to_ab(selected_sed_p50)
selected_sed_mag_p84 = maggies_to_ab(selected_sed_p16)

# Convert model photometry
full_phot_mag_p16 = maggies_to_ab(full_phot_p84)
full_phot_mag_p50 = maggies_to_ab(full_phot_p50)
full_phot_mag_p84 = maggies_to_ab(full_phot_p16)

selected_phot_mag_p16 = maggies_to_ab(selected_phot_p84)
selected_phot_mag_p50 = maggies_to_ab(selected_phot_p50)
selected_phot_mag_p84 = maggies_to_ab(selected_phot_p16)

plt.figure(figsize=(16, 8))

# Posterior SEDs
plt.fill_between(
    full_wspec, full_sed_mag_p16, full_sed_mag_p84,
    color=original_colour, alpha=0.35, label=f"{sample1_name} 16–84%"
)
plt.plot(
    full_wspec, full_sed_mag_p50,
    color=original_colour, lw=1.5, label=f"{sample1_name} posterior median"
)

plt.fill_between(
    selected_wspec, selected_sed_mag_p16, selected_sed_mag_p84,
    color=selected_colour, alpha=0.35, label=f"{sample2_name} 16–84%"
)
plt.plot(
    selected_wspec, selected_sed_mag_p50,
    color=selected_colour, lw=1.5, label=f"{sample2_name} posterior median"
)

# Posterior model photometry
plt.errorbar(
    wphot, full_phot_mag_p50,
    yerr=[
        full_phot_mag_p50 - full_phot_mag_p16,
        full_phot_mag_p84 - full_phot_mag_p50
    ],
    label=f"{sample1_name} median photometry",
    marker="s", markersize=10, ls="",
    markerfacecolor="none", markeredgecolor=original_colour, markeredgewidth=3,
    ecolor=original_colour, alpha=0.6, capsize=3
)

plt.errorbar(
    wphot, selected_phot_mag_p50,
    yerr=[
        selected_phot_mag_p50 - selected_phot_mag_p16,
        selected_phot_mag_p84 - selected_phot_mag_p50
    ],
    label=f"{sample2_name} median photometry",
    marker="s", markersize=10, ls="",
    markerfacecolor="none", markeredgecolor=selected_colour, markeredgewidth=3,
    ecolor=selected_colour, alpha=0.6, capsize=3
)
...
plt.errorbar(
    ...
    ecolor="black", marker="o", markersize=10, ls="",
    markerfacecolor="none", markeredgecolor="black", markeredgewidth=3,
    capsize=3
)
...
all_mags = np.concatenate([
    full_sed_mag_p16[np.isfinite(full_sed_mag_p16)],
    full_sed_mag_p84[np.isfinite(full_sed_mag_p84)],
    selected_sed_mag_p16[np.isfinite(selected_sed_mag_p16)],
    selected_sed_mag_p84[np.isfinite(selected_sed_mag_p84)],
...
gtltdesc = "gt" if (gt_or_lt==">") else "lt"

output_file = (
    f"{output_dir}/"
    f"SED_{selected_zbin.replace(' ', '_')}_"
    f"{cut_param}_gtltdesc_{cut_value}.png"
)
...
print(
    f"Full posterior SEDs:     "
    f"{len(full_wspec)}"
)

print(
    f"Selected posterior SEDs: "
    f"{len(selected_wspec)}"
)
